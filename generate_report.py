...
def read_sheet(sheet_name):
    ...
    # Build HTML table (INLINE styles → email safe)
    html = "<table class='uptime-table' cellpadding='0' cellspacing='0'>"
    html += "<thead><tr>"
    for h in headers:
        html += f"<th>{h}</th>"
    html += "</tr></thead><tbody>"
...
if idx_out is not None and idx_acc is not None:
    max_row = max(
        weekly_rows,
        key=lambda r: downtime_to_minutes(r[idx_out])
    )
    ...
    if downtime_to_minutes(max_row[idx_out]) > 0:
        ...
        major_incident["rca"] = (
            max_row[idx_rca] if idx_rca is not None else ""
        )
